...
class SequentialSearch(SearchBaseClass, ABC):
    ...
    def weighted_a_star_search(self, node_initial):
        # tuple of [f(n), nodeObject]
        fringe = [[self.evaluation_function(node_initial), node_initial]]
        # goal Test
        while len(fringe) != 0:
            node_current = fringe.pop()[1]
            for primitive_successor in node_current.get_successors():
                ...
                if goal_flag:
                    ...
                    return True
                # add all successors to fringe with their evaluation
                fringe.append([self.evaluation_function(child), child])
            # sort the fringe in order to pop the least cost node in the next iteration
            fringe = sorted(fringe, key=lambda tup: tup[0], reverse=True)
        # fringe is empty...cannot go to your destination (no available way)
        return False
    ...
    def execute_search(self, time_pause) -> Tuple[Union[None, List[List[State]]], Union[None, List[MotionPrimitive]], Any]:
        ...
        # Execute Search
        found = self.weighted_a_star_search(self.node_initial)
        return found
